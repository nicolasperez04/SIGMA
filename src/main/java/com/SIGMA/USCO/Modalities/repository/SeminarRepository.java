...
public interface SeminarRepository extends JpaRepository<Seminar, Long> {
    ...
    @Query("SELECT s FROM Seminar s WHERE s.active = true AND s.currentParticipants < s.maxParticipants AND s.status = com.SIGMA.USCO.Modalities.entity.enums.SeminarStatus.OPEN AND s.academicProgram.id = :programId")
    List<Seminar> findActiveWithAvailableSeatsByProgram(@Param("programId") Long programId);

    @Query("SELECT s FROM Seminar s WHERE s.active = true AND s.currentParticipants < s.maxParticipants AND s.status = com.SIGMA.USCO.Modalities.entity.enums.SeminarStatus.OPEN")
